...
class Login
{
    ...
    public $password_reset_mail_successfully_sent = false;

    public $password_reset_failed_but_link_was_valid = false;
    ...
    public function __construct()
    {
        ...
        // if user just submitted a login form
            ...
            $this->loginWithPostData($_POST['user_name'], $_POST['user_pswd'], $_POST['user_rememberme']);
        }
...
private function getUserDataEmail($user_email)
    ...
    public function setPasswordResetDatabaseTokenAndSendMail($user_email)
    {
        ...
        if (empty($user_email)) {
            ...
            // database query, getting all the info of the selected user
            $result_row = $this->getUserDataEmail($user_email);
            ...
            if (isset($result_row->user_id)) {
                ...
                if ($query_update->rowCount() == 1) {
                    ...
                    $this->sendPasswordResetMail($result_row->user_name, $user_email, $user_password_reset_hash);
                    $this->password_reset_mail_successfully_sent = true;
                    return true;
                } else {
                    $this->errors[] = MESSAGE_DATABASE_ERROR;
                }
            } else {
                $this->errors[] = "We can't find this mail address in our database.<br />Please try again!";
            }
        }
    ...
    public function checkIfEmailVerificationCodeIsValid($user_name, $verification_code)
    {
        ...
        if (empty($user_name) || empty($verification_code)) {
            ...
            if (isset($result_row->user_id) && $result_row->user_password_reset_hash == $verification_code) {
                ...
                $this->errors[] = "Message reset link is not valid! <br />Didn't you get another one?";
            }
        }
    }
    ...
    public function editNewPassword($user_name, $user_password_reset_hash, $user_password_new, $user_password_repeat)
    {
        ...
        if (empty($user_name) || empty($user_password_reset_hash) || empty($user_password_new) || empty($user_password_repeat)) {
            $this->errors[] = MESSAGE_PASSWORD_EMPTY;
            $this->password_reset_failed_but_link_was_valid = true;
        // is the repeat password identical to password
